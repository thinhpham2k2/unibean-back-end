...
public class StudentRepository : IStudentRepository
{
    ...
    public Student GetById(string id)
    {
        ...
        try
        {
            ...
            .Include(b => b.Campus)
            .Include(b => b.Account)
            .Include(s => s.Orders.Where(o => o.Status.Equals(true)))
            .Include(s => s.Activities.Where(a => a.Status.Equals(true)))
                .ThenInclude(a => a.Type)
            .Include(s => s.Wallets.Where(w => w.Status.Equals(true)))
                .ThenInclude(w => w.Type)
            .Include(s => s.Wallets.Where(w => w.Status.Equals(true)))
                .ThenInclude(w => w.OrderTransactions)
            .Include(s => s.Wallets.Where(w => w.Status.Equals(true)))
                .ThenInclude(w => w.ActivityTransactions)
            .Include(s => s.Wallets.Where(w => w.Status.Equals(true)))
                .ThenInclude(w => w.ChallengeTransactions)
            .Include(s => s.Wallets.Where(w => w.Status.Equals(true)))
                .ThenInclude(w => w.PaymentTransactions)
            .Include(s => s.Payments.Where(p => p.Status.Equals(true)))
            ...
            .Include(s => s.StudentChallenges.Where(s => s.Status.Equals(true)))
                .ThenInclude(s => s.ChallengeTransactions)
            .FirstOrDefault();
        }
        catch (Exception ex)
        {
